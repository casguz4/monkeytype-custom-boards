...
export function UserStatsDataGrid({ profiles }: Props) {
  ...
  const rows = useMemo<FlatRow[]>(() => {
    return profiles.map((profile) => {
      ...
      const time = Object.fromEntries(
        timeBuckets.map((k) => [k, getBucketBest(profile, "time", k)])
      ) as Record<TimeKey, ReturnType<typeof getBucketBest>>;
      const words = Object.fromEntries(
        wordBuckets.map((k) => [k, getBucketBest(profile, "words", k)])
      ) as Record<WordKey, ReturnType<typeof getBucketBest>>;
  ...
  const SortIcon = ({ col }: { col: Column }) => {
    ...
    if (sortKey !== col.key) return <ArrowUpDown className="h-4 w-4" />;
    return sortDir === "asc" ? (
